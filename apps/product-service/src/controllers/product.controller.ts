...
export const deleteDiscountCodes = async (
    ...
    try {
        ...
        if (discountCode.vendorId !== vendorId) {
...
export const uploadProductImage = async( req: Request,
    res: Response,
    next: NextFunction
) => {
    try {
    ...
    try {
      ...
      const newProduct = await prisma.products.create({
        data: {
          ...
          cash_on_delivery,
          slug,
          shopId: req.vendor.shop?.id!, // ✅ fixed here
          tags: Array.isArray(tags) ? tags : tags.split(","),
          brand,
          video_url,
          category,
...
export const deleteProduct = async (
    ...
    try {
        ...
        const vendorId = req.vendor?.shop?.id
       ...
       const product = await prisma.products.findUnique({
        ...
        select: {
           id: true, shopId: true, isDeleted: true
        }
       }) 
       ...
       res.status(201).json({
       message:
       "Product is scheduled for deletion in 24 hours. You can recover it within this time",
